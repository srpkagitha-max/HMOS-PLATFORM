...
} from "./firebase-service-v2.8.0.js";
...
async function submitAdmission(event){
  ...
  catch(err){console.error("HMOS admission save error",err);const messages={"permission-denied":"Admission save is blocked by Firestore Rules. Publish the V2.8.0 rules.","invalid-admission-details":"Check student and parent details.","institute-session-missing":"Institute session expired. Login again."};m.textContent=messages[err.code]||`Could not save admission. Error: ${err.code||"unknown"}`;m.className="form-message show error form-wide";b.disabled=false;b.innerHTML="Save Admission <span>→</span>";}
}
