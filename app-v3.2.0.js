import {
  ...
  archiveInstitute, restoreInstitute, resetInstitutePassword, renewSubscription, createStudentAdmission, generateStudentId, loginStudent, changeStudentPassword, validateStudentSession, listInstituteStudents, updateStudentProfile, setStudentAccountStatus, listInstituteRooms, createRoom, allotStudentBed, vacateStudentBed, resetStudentPassword, archiveStudentProfile,
  listInstituteFees, saveStudentFeePlan, recordStudentFeePayment, listStudentPayments
} from "./firebase-service-v3.2.0.js";
...
const state = {
  ...
  lastCredentials: null, selectedId: null, search: "", filter: "all", notice: null, latestAdmission: null, studentSession: null, studentCurrentPassword: "", adminStudents: [], adminStudentSearch: "", selectedStudentId: null, rooms: [], selectedRoomId: null, fees: [], selectedFeeStudentId: null, feePayments: []
};
...
function renderInstitutePortal(){const i=state.instituteSession;if(!i){state.screen="institute";return render();}app.innerHTML=shell(`<section class="card portal-card wide-card"><div class="portal-head"><div class="portal-logo">${esc((i.instituteName||"H").slice(0,1).toUpperCase())}</div><div><span class="step success-step">Institute portal</span><h2>${esc(i.instituteName||"Institute Portal")}</h2><p>${esc(i.city||i.address||"HMOS verified institute")} · Code: <strong>${esc(i.instituteCode||"")}</strong></p></div><button id="institute-logout" class="secondary">Logout</button></div><div class="portal-status"><span class="pill active-pill">${esc(i.status||"active")}</span><span>Subscription ends: <strong>${formatDate(i.subscriptionEnd)}</strong></span><button id="refresh-institute-session" class="text-link compact-link" type="button">Refresh access</button></div><div class="portal-actions"><button id="new-admission-card" class="portal-action"><span>＋</span><strong>New Admission</strong><small>Open public admission form</small></button><button id="student-login-card" class="portal-action"><span>🎓</span><strong>Student Login</strong><small>Student ID and password</small></button><button id="admin-login-card" class="portal-action"><span>🛡</span><strong>Admin Login</strong><small>Owner, warden and staff</small></button><button id="room-management-card" class="portal-action"><span>🛏</span><strong>Room Management</strong><small>Rooms, beds and allotment</small></button><button id="fees-management-card" class="portal-action"><span>₹</span><strong>Fees Management</strong><small>Fee plans, payments and receipts</small></button></div><p id="portal-message" class="form-message"></p><div class="portal-contact"><strong>Institute Contact</strong><p>${esc(i.ownerPhone||"Contact not added")}${i.ownerEmail?` · ${esc(i.ownerEmail)}`:""}</p></div></section>`,true);document.querySelector("#institute-logout").onclick=()=>{clearInstituteSession();state.instituteSession=null;state.screen="institute";render();};document.querySelector("#refresh-institute-session").onclick=async e=>{const b=e.currentTarget,m=document.querySelector("#portal-message");b.disabled=true;b.textContent="Checking…";try{state.instituteSession=await validateInstituteSession(i.instituteCode);saveInstituteSession(state.instituteSession,true);m.textContent="Institute access is active and up to date.";m.className="form-message show success-message";}catch(err){clearInstituteSession();state.instituteSession=null;state.screen="institute";return renderInstituteLogin(err.code==="subscription-expired"?"Subscription expired. Contact HMOS support.":"Institute access is no longer active. Please login again.");}finally{if(document.body.contains(b)){b.disabled=false;b.textContent="Refresh access";}}};document.querySelector("#new-admission-card").onclick=()=>{state.screen="new-admission";render();};
  document.querySelector("#student-login-card").onclick=()=>{state.screen="student-login";render();};document.querySelector("#admin-login-card").onclick=()=>{state.screen="institute-admin";render();};document.querySelector("#room-management-card").onclick=()=>{state.screen="room-management";render();};document.querySelector("#fees-management-card").onclick=()=>{state.screen="fees-management";render();};}

function renderNewAdmission(message="") {
  const i=state.instituteSession;
  if(!i){state.screen="institute";return render();}
  const today=new Date().toISOString().slice(0,10);
  const studentId=generateStudentId(i.instituteCode);
  app.innerHTML=shell(`<section class="card admission-card wide-card">
    <button id="back-portal" class="back" type="button">← Institute Portal</button>
    <div class="card-heading"><span class="step success-step">Student onboarding</span><h2>New Admission</h2><p>Create a student admission record for <strong>${esc(i.instituteName)}</strong>.</p></div>
    <form id="admission-form" class="form-grid" novalidate>
      ${field("a-student-id","Student ID","text","Auto generated",studentId,"readonly")}
      ${field("a-student-name","Student Full Name","text","Enter student name","","autocomplete='name'")}
      ${field("a-dob","Date of Birth","date","","","max='${today}'")}
      <label class="field"><span>Gender</span><select id="a-gender" required><option value="">Select gender</option><option>Male</option><option>Female</option><option>Other</option></select></label>
      ${field("a-course","Course / Class","text","Example: B.Tech 1st Year")}
      ${field("a-student-phone","Student Phone","tel","Optional 10-digit number","","inputmode='numeric'")}
      ${field("a-parent-name","Parent / Guardian Name","text","Enter parent name")}
      <label class="field"><span>Relation</span><select id="a-parent-relation"><option>Father</option><option>Mother</option><option>Guardian</option><option>Other</option></select></label>
      ${field("a-parent-phone","Parent Phone","tel","Required 10-digit number","","inputmode='numeric'")}
      ${field("a-emergency-phone","Emergency Phone","tel","Optional 10-digit number","","inputmode='numeric'")}
      ${field("a-aadhaar","Aadhaar Last 4 Digits","text","Optional","","maxlength='4' inputmode='numeric'")}
      ${field("a-joining","Joining Date","date","",today)}
      <label class="field form-wide"><span>Permanent Address</span><textarea id="a-address" placeholder="House number, street, village/city, district"></textarea></label>
      <label class="field form-wide"><span>Notes</span><textarea id="a-notes" placeholder="Medical, food or hostel notes (optional)"></textarea></label>
      <p id="admission-message" class="form-message form-wide ${message?"show error":""}">${esc(message)}</p>
      <button id="admission-submit" class="primary form-wide" type="submit">Save Admission <span>→</span></button>
    </form>
  </section>`,true);
...
async function submitAdmission(event){
  ...
  const m=document.querySelector("#admission-message"),b=document.querySelector("#admission-submit");
  const value=id=>document.querySelector(`#${id}`).value.trim();
  const digits=id=>value(id).replace(/\D/g, "");
  const input={studentId:value("a-student-id"),studentName:value("a-student-name"),dateOfBirth:value("a-dob"),gender:value("a-gender"),courseOrClass:value("a-course"),studentPhone:digits("a-student-phone"),parentName:value("a-parent-name"),parentRelation:value("a-parent-relation"),parentPhone:digits("a-parent-phone"),emergencyPhone:digits("a-emergency-phone"),aadhaarLast4:digits("a-aadhaar"),joiningDate:value("a-joining"),address:value("a-address"),notes:value("a-notes")};
  const optionalPhones=[input.studentPhone,input.emergencyPhone].filter(Boolean);
  if(!input.studentName||!input.gender||!input.parentName||!/^\d{10}$/.test(input.parentPhone)||optionalPhones.some(x=>!/^\d{10}$/.test(x))||(input.aadhaarLast4&&!/^\d{4}$/.test(input.aadhaarLast4))){m.textContent="Complete required fields. Phone numbers must contain 10 digits; Aadhaar field must contain only the last 4 digits.";m.className="form-message show error form-wide";return;}
  b.disabled=true;b.textContent="Saving…";m.textContent="Saving admission securely…";m.className="form-message show info form-wide";
  try{state.latestAdmission=await createStudentAdmission(input,state.instituteSession);state.screen="admission-success";render();}
  catch(err){console.error("HMOS admission save error",err);const messages={"permission-denied":"Admission save is blocked by Firestore Rules. Publish the V2.8.1 rules.","invalid-admission-details":"Check phone numbers, Aadhaar last 4 digits and required details.","institute-session-missing":"Institute session expired. Login again.","duplicate-student-id":"This Student ID already exists. Go back and submit again for a new ID.","student-limit-reached":"Institute student capacity is full. Increase the Student Limit first.","admission-save-timeout":"Network is slow. Admission was not confirmed; try again."};m.textContent=messages[err.code]||`Could not save admission. Error: ${err.code||"unknown"}`;m.className="form-message show error form-wide";b.disabled=false;b.innerHTML="Save Admission <span>→</span>";}
}
...
function renderStudentDashboard(){const s=state.studentSession;if(!s){state.screen="student-login";return render();}app.innerHTML=shell(`<section class="card portal-card wide-card student-dashboard"><div class="portal-head"><div class="portal-logo">${esc((s.studentName||"S")[0].toUpperCase())}</div><div><span class="step success-step">Student portal</span><h2>${esc(s.studentName||"Student")}</h2><p>ID: <strong>${esc(s.studentId)}</strong> · ${esc(s.courseOrClass||"Course not set")}</p></div><button id="student-logout" class="secondary" type="button">Logout</button></div><div class="student-summary-grid"><article><span>Account</span><strong>Active</strong><small>${esc(s.instituteName||s.instituteCode||"")}</small></article><article><span>Room / Bed</span><strong>${esc(s.roomId||"Not allotted")}</strong><small>${esc(s.bedId||"No bed")}</small></article><article><span>Fees</span><strong>${esc(s.feesStatus||"Not set")}</strong><small>${Number(s.feeBalance||0)>0?`Balance ₹${Number(s.feeBalance||0).toLocaleString("en-IN")}`:Number(s.feeTotal||0)>0?"Paid in full":"Current status"}</small></article><article><span>Joining Date</span><strong>${esc(s.joiningDate||"—")}</strong><small>${esc(s.parentName||"")}</small></article></div><div class="portal-contact"><strong>Contact</strong><p>${esc(s.studentPhone||s.parentPhone||"Contact not added")}</p></div></section>`,true);document.querySelector("#student-logout").onclick=()=>{clearStudentSession();state.screen="institute-portal";render();};}
...
function roomCards(){
  if(!state.rooms.length)return `<div class="empty-state"><strong>No rooms created yet.</strong><p>Create the first room and HMOS will generate beds automatically.</p></div>`;
  return `<div class="room-list">${state.rooms.map(r=>`<article class="room-card"><div><span class="step">${esc(r.building||"Building")} · ${esc(r.floor||"Floor")}</span><h3>Room ${esc(r.roomNumber)}</h3><p>${esc(r.roomType||"Room")} · ${Number(r.capacity||0)} beds</p></div><div class="room-stats"><strong>${Number(r.occupiedBeds||0)}/${Number(r.capacity||0)}</strong><small>Occupied</small></div><button class="row-menu" data-room-manage="${esc(r.id||r.roomId)}">Manage</button></article>`).join("")}</div>`;
}
async function renderRoomManagement(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><div class="dashboard-head"><div><span class="step success-step">Hostel operations</span><h2>Room & Bed Management</h2><p>${esc(i.instituteName)} · ${esc(i.instituteCode)}</p></div><button id="room-back-portal" class="secondary">Institute Portal</button></div><div class="metric-grid"><article><span>Total Rooms</span><strong id="room-total">—</strong><small>Active rooms</small></article><article><span>Total Beds</span><strong id="bed-total">—</strong><small>Generated beds</small></article><article><span>Vacant Beds</span><strong id="bed-vacant">—</strong><small>Ready to allot</small></article></div><div class="section-title"><div><h3>Rooms</h3><p>Create rooms and allot students to vacant beds.</p></div><button id="open-create-room" class="primary compact-primary">+ Create Room</button></div><p id="room-message" class="form-message"></p><div id="room-results"><div class="loading-card"><div class="loader"></div><p>Loading rooms…</p></div></div></section>`,true);
  document.querySelector("#room-back-portal").onclick=()=>{state.screen="institute-portal";render();};
  ...
  try{state.rooms=await listInstituteRooms(i.instituteCode);const totalBeds=state.rooms.reduce((n,r)=>n+Number(r.capacity||0),0),occupied=state.rooms.reduce((n,r)=>n+Number(r.occupiedBeds||0),0);document.querySelector("#room-total").textContent=state.rooms.length;document.querySelector("#bed-total").textContent=totalBeds;document.querySelector("#bed-vacant").textContent=totalBeds-occupied;document.querySelector("#room-results").innerHTML=roomCards();document.querySelectorAll("[data-room-manage]").forEach(b=>b.onclick=()=>{state.selectedRoomId=b.dataset.roomManage;state.screen="room-manage";render();});}catch(err){const m=document.querySelector("#room-message");m.textContent=`Could not load rooms. ${err.code||""}`;m.className="form-message show error";}
}
function renderRoomManage(){
  ...
  const vacant=(r.beds||[]).filter(b=>b.status!=="occupied");
  const unallotted=state.adminStudents.filter(s=>!s.roomId);
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><button id="room-manage-back" class="back">← Room Management</button><div class="manage-head"><div class="avatar large">${esc(r.roomNumber)}</div><div class="manage-title"><span class="step">${esc(r.building)} · ${esc(r.floor)}</span><h2>Room ${esc(r.roomNumber)}</h2><p>${esc(r.roomType)} · ${Number(r.occupiedBeds||0)}/${Number(r.capacity||0)} occupied</p></div></div><div class="bed-grid">${(r.beds||[]).map(b=>`<article class="bed-card ${b.status==="occupied"?"occupied-bed":"vacant-bed"}"><span>Bed ${esc(b.bedNumber)}</span><strong>${b.status==="occupied"?esc(b.studentName||b.studentId):"Vacant"}</strong>${b.status==="occupied"?`<button data-vacate-student="${esc(b.studentId)}" class="secondary">Vacate</button>`:""}</article>`).join("")}</div><div class="section-title"><div><h3>Allot a Bed</h3><p>Select an unallotted student and vacant bed.</p></div></div><form id="bed-allot-form" class="form-grid"><label class="field"><span>Student</span><select id="allot-student"><option value="">Select student</option>${unallotted.map(s=>`<option value="${esc(s.studentId)}">${esc(s.studentName)} · ${esc(s.studentId)}</option>`).join("")}</select></label><label class="field"><span>Vacant Bed</span><select id="allot-bed"><option value="">Select bed</option>${vacant.map(b=>`<option value="${esc(b.bedNumber)}">Bed ${esc(b.bedNumber)}</option>`).join("")}</select></label><p id="allot-message" class="form-message form-wide"></p><button id="allot-save" class="primary form-wide">Allot Bed</button></form></section>`,true);
  document.querySelector("#room-manage-back").onclick=()=>{state.screen="room-management";render();};
  document.querySelector("#bed-allot-form").onsubmit=async e=>{e.preventDefault();const b=document.querySelector("#allot-save"),m=document.querySelector("#allot-message");b.disabled=true;b.textContent="Allotting…";try{await allotStudentBed({studentIdValue:document.querySelector("#allot-student").value,roomIdValue:r.id||r.roomId,bedNumberValue:document.querySelector("#allot-bed").value,instituteCodeValue:i.instituteCode});state.adminStudents=await listInstituteStudents(i.instituteCode);state.rooms=await listInstituteRooms(i.instituteCode);renderRoomManage();}catch(err){m.textContent=`Could not allot bed. ${err.code||""}`;m.className="form-message show error";b.disabled=false;b.textContent="Allot Bed";}};
  document.querySelectorAll("[data-vacate-student]").forEach(b=>b.onclick=async()=>{if(!confirm("Vacate this bed?"))return;b.disabled=true;try{await vacateStudentBed(b.dataset.vacateStudent,i.instituteCode);state.adminStudents=await listInstituteStudents(i.instituteCode);state.rooms=await listInstituteRooms(i.instituteCode);renderRoomManage();}catch(err){alert(`Could not vacate bed. ${err.code||""}`);b.disabled=false;}});
}


async function renderFeesManagement(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><button id="fees-back" class="back">← Institute Portal</button><div class="card-heading"><span class="step">Finance operations</span><h2>Fees Management</h2><p>${esc(i.instituteName)} · Set student fees, collect payments and issue receipts.</p></div><div class="loading-card"><div class="loader"></div><p>Loading fee records…</p></div></section>`,true);
  try{const [students,fees]=await Promise.all([listInstituteStudents(i.instituteCode),listInstituteFees(i.instituteCode)]);state.adminStudents=students;state.fees=fees;renderFeesManagementReady();}catch(err){app.innerHTML=shell(`<section class="card login-card"><button id="fees-back" class="back">← Institute Portal</button><h2>Fees could not load</h2><p class="form-message show error">${esc(err.code||"fee-load-error")}</p></section>`,true);document.querySelector("#fees-back").onclick=()=>{state.screen="institute-portal";render();};}
}
...
function renderFeesManagementReady(){
  ...
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><button id="fees-back" class="back">← Institute Portal</button><div class="card-heading"><span class="step">Finance operations</span><h2>Fees Management</h2><p>${esc(i.instituteName)} · Student fee ledger</p></div><div class="metric-grid"><article><span>Total Fees</span><strong>₹${total.toLocaleString("en-IN")}</strong><small>Assigned</small></article><article><span>Collected</span><strong>₹${paid.toLocaleString("en-IN")}</strong><small>Payments received</small></article><article><span>Balance</span><strong>₹${balance.toLocaleString("en-IN")}</strong><small>Outstanding</small></article></div><div class="section-title"><div><h3>Student Fee Accounts</h3><p>Set fee plan or record a payment.</p></div></div><div class="fee-student-list">${state.adminStudents.map(s=>{const f=feeFor(s.studentId),b=Number(f?.balanceAmount||0);return `<article class="fee-student-card"><div><strong>${esc(s.studentName)}</strong><span>${esc(s.studentId)} · ${esc(s.courseOrClass||"")}</span></div><div class="fee-amounts"><small>Total</small><strong>₹${Number(f?.totalFee||0).toLocaleString("en-IN")}</strong><small>Balance</small><strong class="${b>0?'due-text':'health-text'}">₹${b.toLocaleString("en-IN")}</strong></div><button class="secondary" data-fee-manage="${esc(s.studentId)}">Manage Fees</button></article>`}).join("")||'<div class="empty-state"><strong>No students found.</strong></div>'}</div></section>`,true);
  document.querySelector("#fees-back").onclick=()=>{state.screen="institute-portal";render();};document.querySelectorAll("[data-fee-manage]").forEach(b=>b.onclick=()=>openStudentFeeModal(b.dataset.feeManage));
}
function openStudentFeeModal(studentId){
  const s=state.adminStudents.find(x=>x.studentId===studentId),f=feeFor(studentId)||{};if(!s)return;
...
function render(){if(state.screen==="super-admin")return renderSuperAdmin();if(state.screen==="institute-password-change")return renderInstitutePasswordChange();if(state.screen==="dashboard")return renderAdminDashboard();if(state.screen==="create")return renderCreate();if(state.screen==="manage")return renderManage();if(state.screen==="institute-portal")return renderInstitutePortal();if(state.screen==="new-admission")return renderNewAdmission();if(state.screen==="admission-success")return renderAdmissionSuccess();if(state.screen==="student-login")return renderStudentLogin();if(state.screen==="student-password-change")return renderStudentPasswordChange();if(state.screen==="student-dashboard")return renderStudentDashboard();if(state.screen==="institute-admin")return renderInstituteAdmin();if(state.screen==="student-manage")return renderStudentManage();if(state.screen==="room-management")return renderRoomManagement();if(state.screen==="room-manage")return renderRoomManage();if(state.screen==="fees-management")return renderFeesManagement();return renderInstituteLogin();}
...
ensureInstallButton();
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker-v3.1.0.js", { scope: "./", updateViaCache: "none" }).catch(()=>{}));
