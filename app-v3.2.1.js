...
} from "./firebase-service-v3.2.1.js";
...
const state = {
  ...
  lastCredentials: null, selectedId: null, search: "", filter: "all", notice: null, latestAdmission: null, studentSession: null, studentCurrentPassword: "", adminStudents: [], adminStudentSearch: "", selectedStudentId: null, rooms: [], selectedRoomId: null, selectedFloor: "", selectedBedNumber: "", fees: [], selectedFeeStudentId: null, feePayments: []
};
...
async function renderRoomManagement(){
  const i=state.instituteSession;if(!i){state.screen="institute";return render();}
  app.innerHTML=shell(`<section class="card dashboard-card wide-card"><div class="dashboard-head"><div><span class="step success-step">Hostel operations</span><h2>Movie-Style Bed Selection</h2><p>${esc(i.instituteName)} · ${esc(i.instituteCode)}</p></div><button id="room-back-portal" class="secondary">Institute Portal</button></div><div class="metric-grid"><article><span>Total Rooms</span><strong id="room-total">—</strong><small>Active rooms</small></article><article><span>Visible Beds</span><strong id="bed-total">—</strong><small>Shown for allotment</small></article><article><span>Vacant Beds</span><strong id="bed-vacant">—</strong><small>Ready to allot</small></article></div><div class="section-title"><div><h3>Select Floor</h3><p>Choose floor, room and bed just like booking a movie ticket.</p></div><button id="open-create-room" class="primary compact-primary">+ Create Room</button></div><div id="floor-tabs" class="floor-tabs"></div><p id="room-message" class="form-message"></p><div id="room-results"><div class="loading-card"><div class="loader"></div><p>Loading rooms…</p></div></div></section>`,true);
