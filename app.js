import {
  loginSuperAdmin, logoutCurrentUser, watchAuth, listInstitutes, createInstitute, loginInstitute, changeInstitutePassword,
...
const INSTITUTE_SESSION_KEY = "hmosInstituteSessionV26";
...
function renderInstituteLogin(message=""){
  app.innerHTML=shell(`<section class="card login-card"><div class="card-heading"><span class="step">Institute access</span><h2>Institute Login</h2><p>Enter credentials issued by HMOS.</p></div><form id="institute-form">${field("institute-id","Institute Code / ID","text","Example: ABCO1234")}${field("institute-password","Institute Password","password","Enter password","","autocomplete='current-password'")}<label class="check"><input type="checkbox" id="remember-institute"/><span>Remember this institute on this device</span></label><p id="form-message" class="form-message ${message?"show error":""}">${esc(message)}</p><button class="primary">Continue <span>→</span></button></form><button id="super-admin-link" class="text-link">Super Admin Login</button></section>`);
  document.querySelector("#super-admin-link").onclick=()=>{state.screen="super-admin";render();};
  document.querySelector("#institute-form").onsubmit=async e=>{e.preventDefault();const b=e.currentTarget.querySelector("button"),m=document.querySelector("#form-message"),code=document.querySelector("#institute-id").value,password=document.querySelector("#institute-password").value,remember=document.querySelector("#remember-institute").checked;b.disabled=true;b.textContent="Checking…";try{state.instituteSession=await loginInstitute(code,password);state.instituteCurrentPassword=password;if(state.instituteSession.mustChangePassword){state.screen="institute-password-change";}else{saveInstituteSession(state.instituteSession,remember);state.screen="institute-portal";}render();}catch(err){m.textContent={"invalid-institute-credential":"Incorrect institute code or password.","institute-inactive":"This institute account is inactive.","subscription-expired":"Subscription expired. Contact HMOS support.","missing-credentials":"Enter institute code and password."}[err.code]||"Institute login failed.";m.className="form-message show error";b.disabled=false;b.innerHTML="Continue <span>→</span>";}};
}
...
function renderInstitutePortal(){const i=state.instituteSession;if(!i){state.screen="institute";return render();}app.innerHTML=shell(`<section class="card portal-card wide-card"><div class="portal-head"><div class="portal-logo">${esc((i.instituteName||"H").slice(0,1).toUpperCase())}</div><div><span class="step success-step">Institute portal</span><h2>${esc(i.instituteName||"Institute Portal")}</h2><p>${esc(i.city||i.address||"HMOS verified institute")} · Code: <strong>${esc(i.instituteCode||"")}</strong></p></div><button id="institute-logout" class="secondary">Logout</button></div><div class="portal-status"><span class="pill active-pill">${esc(i.status||"active")}</span><span>Subscription ends: <strong>${formatDate(i.subscriptionEnd)}</strong></span></div><div class="portal-actions"><button id="new-admission-card" class="portal-action"><span>＋</span><strong>New Admission</strong><small>Open public admission form</small></button><button id="student-login-card" class="portal-action"><span>🎓</span><strong>Student Login</strong><small>Student ID and password</small></button><button id="admin-login-card" class="portal-action"><span>🛡</span><strong>Admin Login</strong><small>Owner, warden and staff</small></button></div><p id="portal-message" class="form-message"></p><div class="portal-contact"><strong>Institute Contact</strong><p>${esc(i.ownerPhone||"Contact not added")}${i.ownerEmail?` · ${esc(i.ownerEmail)}`:""}</p></div></section>`,true);document.querySelector("#institute-logout").onclick=()=>{clearInstituteSession();state.instituteSession=null;state.screen="institute";render();};["new-admission-card","student-login-card","admin-login-card"].forEach(id=>document.querySelector(`#${id}`).onclick=()=>{const m=document.querySelector("#portal-message");m.textContent=id==="new-admission-card"?"Admission module will open in the next build.":id==="student-login-card"?"Student login foundation is scheduled for V2.8.":"Admin role login foundation is scheduled for V2.7.";m.className="form-message show info";});}
...
window.addEventListener("hmos:institute-save-result", event => {
  const detail = event.detail || {};
  if (detail.ok) {
    console.info("HMOS institute save confirmed:", detail.instituteId);
    return;
  }
  const message = detail.code === "permission-denied"
    ? "Background save failed: Firestore permission denied."
    : `Background save failed (${detail.code || "unknown-error"}). Reopen Edit and retry.`;
  state.notice = { message, type: "error" };
  if (state.screen === "dashboard") renderAdminDashboard();
});
...
ensureInstallButton();
if("serviceWorker" in navigator)window.addEventListener("load",()=>navigator.serviceWorker.register("./service-worker.js?v=2.6.3", { scope: "./" }).catch(()=>{}));
