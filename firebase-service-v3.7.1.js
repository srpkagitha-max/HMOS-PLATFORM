...
export async function listInstituteStudents(instituteCodeValue) {
  ...
  const snap = await withTimeout(getDocs(collection(db, "students")), 12000, "student-list-timeout");
  return snap.docs
    .map(item => ({ id: item.id, ...item.data() }))
    .filter(item => normalizeCode(item.instituteCode) === instituteCode)
...
export async function listInstituteRooms(instituteCodeValue) {
  const instituteCode = normalizeCode(instituteCodeValue);
  if (!instituteCode) throw Object.assign(new Error("Institute code missing"), { code: "institute-session-missing" });
  const snap = await withTimeout(getDocs(collection(db, "rooms")), 12000, "room-list-timeout");
  return snap.docs.map(d => ({ id: d.id, ...d.data() }))
    .filter(r => normalizeCode(r.instituteCode) === instituteCode)
...
export async function listInstituteFees(instituteCodeValue){
  const instituteCode=normalizeCode(instituteCodeValue);
  const snap=await withTimeout(getDocs(collection(db,"fees")),12000,"fee-list-timeout");
  return snap.docs.map(d=>({id:d.id,...d.data()})).filter(f=>normalizeCode(f.instituteCode)===instituteCode);
}
...
export async function listStudentPayments(studentIdValue){
  const studentId=normalizeCode(studentIdValue);const snap=await withTimeout(getDocs(collection(db,"payments")),12000,"payment-list-timeout");
  return snap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>normalizeCode(p.studentId)===studentId);
}
...
export async function listPendingAdmissions(instituteCodeValue){
  const instituteCode=normalizeCode(instituteCodeValue);
  const snap=await withTimeout(getDocs(collection(db,"pendingAdmissions")),12000,"pending-list-timeout");
  return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===instituteCode && x.status==="pending_payment_verification");
}
...
export async function listInstituteMealAttendance(instituteCodeValue,dateValue){const code=normalizeCode(instituteCodeValue),date=todayKey(dateValue),snap=await getDocs(collection(db,"mealAttendance"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code&&x.date===date);}
...
export async function listInstituteMovements(instituteCodeValue){const code=normalizeCode(instituteCodeValue),snap=await getDocs(collection(db,"movements"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code).sort((a,b)=>String(b.createdAt?.seconds||0).localeCompare(String(a.createdAt?.seconds||0)));}
export async function getStudentMovements(studentIdValue){const id=normalizeCode(studentIdValue),snap=await getDocs(collection(db,"movements"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.studentId)===id);}
...
export async function listStudentComplaints(studentIdValue){const id=normalizeCode(studentIdValue),snap=await getDocs(collection(db,"complaints"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.studentId)===id);}
export async function listInstituteComplaints(instituteCodeValue){const code=normalizeCode(instituteCodeValue),snap=await getDocs(collection(db,"complaints"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code);}
...
export async function saveInstituteBranding(input){
  ...
  const data={instituteCode,instituteId:cleanText(input.instituteId),instituteName:cleanText(input.instituteName),shortName:cleanText(input.shortName),logoUrl:cleanText(input.logoUrl),primaryColor:cleanText(input.primaryColor)||"#0b4f8a",secondaryColor:cleanText(input.secondaryColor)||"#16866b",contactNumber:cleanText(input.contactNumber).replace(/\D/g,""),upiId:cleanText(input.upiId),welcomeMessage:cleanText(input.welcomeMessage),updatedAt:serverTimestamp()};
...
export async function listAuditLogs(instituteCodeValue){const code=normalizeCode(instituteCodeValue),snap=await getDocs(collection(db,"auditLogs"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));}
...
export async function listNotifications({instituteCode,recipientType,recipientId=""}){const code=normalizeCode(instituteCode),rid=normalizeCode(recipientId),snap=await getDocs(collection(db,"notifications"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code&&x.recipientType===recipientType&&(!rid||normalizeCode(x.recipientId)===rid)).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));}
...
export async function listApprovalRequests(instituteCodeValue,statusValue="pending"){const code=normalizeCode(instituteCodeValue),snap=await getDocs(collection(db,"approvalRequests"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code&&(!statusValue||x.status===statusValue)).sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0));}
...
export async function listRecycleBin(instituteCodeValue){const code=normalizeCode(instituteCodeValue),snap=await getDocs(collection(db,"deletedRecords"));return snap.docs.map(d=>({id:d.id,...d.data()})).filter(x=>normalizeCode(x.instituteCode)===code&&x.status==="deleted");}
