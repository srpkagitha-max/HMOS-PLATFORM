...
export async function submitPendingAdmission(input, instituteSession) {
  ...
  if (!cleanText(input.studentName) || !cleanText(input.parentName) || !/^\d{10}$/.test(cleanText(input.parentPhone).replace(/\D/g,"")) || totalFees < 0 || amountPayingNow <= 0 || amountPayingNow > totalFees) throw Object.assign(new Error("Invalid admission details"), {code:"invalid-admission-details"});
