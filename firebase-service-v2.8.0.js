...
export async function createStudentAdmission(input, instituteSession) {
  ...
  if (!instituteCode || !instituteSession?.instituteId) {
  ...
  const record = {
    admissionId,
    studentId,
    instituteId: instituteSession.instituteId,
    instituteCode,
    ...
    studentPhone: cleanText(input.studentPhone),
    parentName,
    parentRelation: cleanText(input.parentRelation),
    parentPhone,
    emergencyPhone: cleanText(input.emergencyPhone),
    aadhaarLast4: cleanText(input.aadhaarLast4),
    joiningDate: cleanText(input.joiningDate) || new Date().toISOString().slice(0,10),
    address: cleanText(input.address),
  ...
  await setDoc(doc(db, "admissions", admissionId), record);
